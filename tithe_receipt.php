...
$client_ip_address = getUserIpAddr(); 
//echo $client_printer_path = '\\\\'.$client_ip_address.'\\'.$printer_model_name;
//$connector = new NetworkPrintConnector($client_ip_address);
//$connector = new FilePrintConnector($client_ip_address, 9100);
//$connector = new WindowsPrintConnector($client_printer_path);
//$connector = new WindowsPrintConnector($printer_model_name);
//$connector = new WindowsPrintConnector("smb://".$client_ip_address);
// $connector = $connector = new WindowsPrintConnector($client_ip_address.'/'.$printer_model_name);
$connector = $connector = new WindowsPrintConnector("smb://Guest@".$client_ip_address.'/'.$printer_model_name);
// $connector = $connector = new WindowsPrintConnector($printer_model_name);
// $connector = new WindowsPrintConnector('smb://'. $client_ip_address .'/'.$printer_model_name);
...
$paid_amount = number_format($PayAmount, 2); 
$cahsier = new item('Cashier: '.$Cashier);
$total = new item('Amount Paid', $paid_amount, true);
/* Date is kept the same for testing */
// $date = date('l jS \of F Y h:i:s A');
$date = date("l jS \of F Y h:i:s A");

/* Start the printer */
$logo = EscposImage::load("assets/img/logo_sm.png", false);
